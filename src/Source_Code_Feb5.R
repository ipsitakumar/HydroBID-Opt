...
rFmedian<-colSums(rFmedian_by_res)^4

rFmean_by_res<-apply(rF,MARGIN=c(1,2),mean)
rFmean<-colSums(rFmean_by_res)

Title<-c(" ", "Total Cost", "Annual Average Cost")



############ THIS PART IS TO CREATE THE TABLE WITH ALL THE RESULTS
Cost_Res_rmt<-rQ*costQ_rmt
Cost_Res_rmt <- round(Cost_Res_rmt, digits = 4)
Cost_Res_rt <- apply(Cost_Res_rmt, MARGIN=c(1,3), sum)

Cost_Res_r<-rowSums(Cost_Res_rt)
Res_name<-as.matrix(resname)
Cost_Res_r_annual_avg<-Cost_Res_r*(12/nT)
Cost_Res_r_annual_avg<-round(Cost_Res_r_annual_avg, digits = 4)


## Each Reservoir
Cost_from_each_reservoir_ALL<-cbind(Res_name, Cost_Res_r, Cost_Res_r_annual_avg)
Total_Cost_Supply_Res<-sum(Cost_Res_r)
Annual_Avg_Cost_Supply_Res<-sum(Cost_Res_r_annual_avg)

## Total Cost from Reservoirs 
Total_Cost_Supply_Res_ALL<-cbind("Total Cost of Supply from Reservoirs",Total_Cost_Supply_Res, Annual_Avg_Cost_Supply_Res)
...
Supply_Cost<-sum(Cost_Res_r,Cost_Imp_j)
Annual_Avg_Supply_Cost<-sum(Annual_Avg_Cost_Supply_Res, Annual_Avg_Cost_Supply_Imp)

## Total Cost of water Supply 
Total_Cost_of_water_supply<-cbind("Total Cost of Water Supply from all sources", Supply_Cost, Annual_Avg_Supply_Cost)
...
Total_Cost_mean<-sum(Supply_Cost, mean_total_Failure)
Annual_Average_Cost_mean<-sum(Annual_Avg_Supply_Cost, mean_total_Failure)
...
Total_Cost_median<-sum(Supply_Cost, median_total_Failure)
Annual_Average_Cost_median<-sum(Annual_Avg_Supply_Cost, median_total_Failure)
...
### TABLE
Table_Results <- rbind(Cost_from_each_reservoir_ALL, Cost_from_each_Import_ALL, Total_Cost_Failure_mean, Total_Cost_Failure_median, Total_Cost_Supply_Res_ALL, Total_Cost_Supply_Imp_ALL, Total_Cost_of_water_supply, Total_Cost_All_mean, Total_Cost_All_median)
colnames(Table_Results)<-Title
...
### 5. Cost
Total_Res_Cost_t<-colSums(Cost_Res_rt)
Total_Imp_Cost_t<-colSums(Cost_Imp_jt)
Total_SS_Cost_t<-rbind(Total_Res_Cost_t, Total_Imp_Cost_t)
Total_SS_Cost_t<-colSums(Total_SS_Cost_t)
Total_Cost_mean_t<-rbind(Total_SS_Cost_t, rF_mean_t)
Total_Cost_mean_t<-colSums(Total_Cost_mean_t)
Total_Cost_median_t<-rbind(Total_SS_Cost_t, rF_median_t)
Total_Cost_median_t<-colSums(Total_Cost_median_t)
Cost_t<-rbind(Cost_Res_rt, Cost_Imp_jt, rF_mean_t, rF_median_t, Total_Res_Cost_t, Total_Imp_Cost_t, Total_SS_Cost_t, Total_Cost_mean_t, Total_Cost_median_t)
...
row.names(Cost_t)<-row_names_cost
